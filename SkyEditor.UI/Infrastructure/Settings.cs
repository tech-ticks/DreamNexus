...
namespace SkyEditorUI.Infrastructure
{
    ...
    public class Settings
    {
        ...
        public static string DataPath
        {
            get
            {
                // The application data folder returned on Mac is just flat out wrong
                // https://github.com/dotnet/runtime/issues/40353
                var appDataFolder = OperatingSystem.IsMacOS()
                    ? Path.Combine(
                        Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Library/Application Support")
                    : Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);

                return Path.Combine(appDataFolder, "DreamNexus");
            }
        }
